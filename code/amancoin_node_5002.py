...
class Blockchain:  # Helps to create blocks
    ...
    def add_transaction(self, sender, receiver, amount):
        ...
        return previous_block[
                   'index'] + 1  # updates the block index by 1.This new block will have the transactions in it.
    ...
    def replace_chain(self):  # used to find the longest chain.
        ...
        for node in network:
            response = requests.get(
                f'http://{node}/get_chain')  # f-string syntax introduced in python 3.6. prints the  ip address
            if response.status_code == 200:  # Success code for http. only for positive executions of requests we will consider
                ...
                if length > max_length and self.is_chain_valid(
                        chain):  # if this node's length is greater than the max length &
...
def mine_block():
    ...
    proof = blockchain.proof_of_work(
        previous_proof)  # proof of new block is calculated based on the previous block's proof of work
    previous_hash = blockchain.hash(previous_block)  # calculates the hexadecimal hash
    blockchain.add_transaction(sender=node_address, receiver='Ayaan', amount=5)  # 5 coins as incentive to the miner
    ...
    response = {'message': 'The block was sucessfully mined.',
                ...
                'transactions': block[
                    'transactions']}  # respose is the message we type if the block is mined
...
def add_transaction():
    ...
    response = {
        'message': f'This transaction will be added to Block {index}'}  # (which is a dictionary), and send it to the method
